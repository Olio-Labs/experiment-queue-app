...
# Cache keyed by date string 'YYYY-MM-DD' -> (ts, (overlays_by_box_number, overlay_errors))
_overlays_by_date_cache: Dict[str, Tuple[float, Tuple[Dict[int, List[Dict[str, Any]]], List[str]]]] = {}
...
_CSS_COLOR_ALLOWLIST: Set[str] = {
    ...
    'cyan', 'magenta', 'teal', 'navy', 'maroon', 'olive',
    'silver', 'gold',
    'peach',
...
def _sanitize_syringe_color_css(raw_color: Any) -> Tuple[Optional[str], Optional[str]]:
    ...
    if s_norm in _CSS_COLOR_ALLOWLIST:
        return s_norm, None
    # User preference: if it's not a real CSS color, just don't show a stripe (no error).
    return None, None
...
def fetch_box_overlays_for_date(
    ...
    """
    Fetch experiment_planner rows for a specific date and aggregate per box_number overlays
    of (manipulation_id, syringe_color).

    Returns:
        overlays_by_box_number: {box_number: [{manipulation_id, syringe_color, syringe_color_css}, ...]}
        overlay_errors: list of human-readable error strings (shown in UI; not silent)
    """
    ...
    # Lightweight cache keyed by date. Keep TTL short to avoid hiding changes while still
    # avoiding repeated reads per request burst.
    now = time.time()
    ...
    if not api_key or not base_id:
        overlay_errors.append("Missing AIRTABLE_API_KEY or AIRTABLE_BASE_ID; cannot load manipulation/syringe overlays.")
    ...
    try:
        ...
        try:
            ...
            overlay_errors.append(f"Could not load manipulations table to map manipulation IDs: {e}")
        ...
        fields = ['start_date', 'experiment_id', 'box_id', 'manipulation_', 'syringe_color']
        ...
        for rec in planner_records:
            ...
            if not box_id:
                overlay_errors.append(f"experiment_planner record {rec_id}: missing/invalid box_id for date {date_str}.")
                continue
            box_number = _box_number_from_box_id(box_id)
            if box_number < 0:
                overlay_errors.append(f"experiment_planner record {rec_id}: could not parse box_number from box_id '{box_id}'.")
                continue
            ...
            if not isinstance(manip_links, list) or not manip_links:
                overlay_errors.append(f"experiment_planner record {rec_id}: missing manipulation_ for box {box_id} on date {date_str}.")
                manip_links = []
            ...
            if syringe_raw is None:
                ...
                colors = [c for c in syringe_raw if c is not None and str(c).strip() != '']
            else:
            ...
            if not manip_links:
                # Still render an "unknown" segment so the box visibly indicates something is missing.
                for raw_color in colors:
                    raw_token = str(raw_color).strip().lower() if raw_color is not None else ''
                    ...
                    overlays_by_box_number[box_number].append({
                        ...
                        'syringe_color_pattern': 'hearts' if raw_token == 'hearts' else None,
                    })
                continue
            ...
            for manip_record_id in manip_links:
                ...
                if not manip_custom:
                    overlay_errors.append(
                        f"experiment_planner record {rec_id}: could not map manipulation_ record id to custom manipulation_id for box {box_id} on date {date_str}."
                    )
                ...
                for raw_color in colors:
                    raw_token = str(raw_color).strip().lower() if raw_color is not None else ''
                    css_color, color_err = _sanitize_syringe_color_css(raw_color)
                    key = (manip_custom, str(raw_color) if raw_color is not None else "")
                    if key in seen_by_box[box_number]:
                        continue
                    seen_by_box[box_number].add(key)
                    overlays_by_box_number[box_number].append({
        ...
        for bn in overlays_by_box_number.keys():
            overlays_by_box_number[bn].sort(key=lambda o: (str(o.get('manipulation_id') or ''), str(o.get('syringe_color') or '')))

    except Exception as e:
        overlay_errors.append(f"Error loading box overlays from experiment_planner ({date_str}): {e}")
...
def fetch_box_room_data(
    ...
    Returns:
        boxes_by_number: { box_number: {
            ...
            'today_overlays': List[dict]  # optional; for today's manipulation/syringe stripes
        }}
    ...
    # Use experiment_planner (selected day) as the source of truth for which cages were in which boxes.
    ...
    for rec in planner_records:
        ...
        if not box_id:
            # Fallback: if we have a linked box_ record id, resolve box_id via the boxes table later.
    ...
    if missing_box_id_box_record_ids:
        ...
        for chunk in _chunked(sorted(missing_box_id_box_record_ids), 20):
            ...
            try:
                ...
                logger.exception("Failed to resolve box_id from boxes table via box_ links")

    # Hydrate cages from cages table using linked record ids.
    cage_record_ids: List[str] = sorted({cid for s in box_number_to_cage_record_ids.values() for cid in s})
    cage_by_record_id: Dict[str, Dict[str, Any]] = {}
    if cage_record_ids:
        cages_table = Api(api_key).table(base_id, 'cages')
        cage_fields = ['cage', 'sex', 'n_mice', 'flagged_issues', 'flagged_issues_history']
    ...
    overlays_by_box_number, overlay_errors = fetch_box_overlays_for_date(api_key, base_id, date_str, exp_filter or None)
    for bn, overlays in overlays_by_box_number.items():
        ...
        boxes_by_number[bn]['today_overlays_css'] = [o for o in overlays if o.get('syringe_color_css')]

    return boxes_by_number, boxes_with_issues_sorted, overlay_errors


def fetch_cages_without_box_data(api_key: str, base_id: str, filter_taconic_only: bool = True) -> Tuple[List[Dict], List[str]]:
    """
    Fetch alive cages without box_id assigned and their flagged issues.
    
    Args:
        filter_taconic_only: If True, only return cages with 'bought_from' containing 'taconic'
...
def get_box_flagged_issues_history(
    ...
    if not api_key or not base_id:
        raise ValueError("AIRTABLE_API_KEY or AIRTABLE_BASE_ID not found in environment")
    ...
    base_formula = f"AND({{box_id}} = '{target_box_id}', IS_SAME({{start_date}}, '{date_str}', 'day'))"
    exp_filter = (experiment_id_filter or '').strip()
    formula = f"AND({base_formula}, {{experiment_id}} = '{exp_filter}')" if exp_filter else base_formula
...
def get_experiment_ids_for_start_date(api_key: str, base_id: str, date_str: str) -> Tuple[List[str], Optional[str]]:
    """
    Return unique experiment_id values from experiment_planner for a given start_date (YYYY-MM-DD).
    Used for displaying a "today's experiment" id in the UI.

    Returns (experiment_ids_sorted, error_or_None). Never raises for missing/multiple; caller decides UI.
    """
    try:
        date_str = str(date_str).strip()
        _parse_yyyy_mm_dd(date_str)
    ...
    try:
        ...
        if len(ids_sorted) > 1:
            return ids_sorted, f"Multiple experiment_id values for start_date {date_str}: {', '.join(ids_sorted)}"
        return ids_sorted, None
    except Exception as e:
        return [], f"Error loading experiment_id for start_date {date_str}: {e}"


def get_start_date_for_experiment_id(api_key: str, base_id: str, experiment_id: str) -> Tuple[Optional[str], Optional[str]]:
    """
    Resolve the unique start_date (YYYY-MM-DD) for a given experiment_id by querying experiment_planner.
    ...
    try:
        ...
        if len(dates_sorted) > 1:
            return None, f"Multiple start_date values for experiment_id {exp}: {', '.join(dates_sorted)}"
...
def _try_find_recent_video_in_hour(
    ...
    """
    Find the most recent usbcam-0.mp4 for a given experiment/box/cage whose timestamp (from filename)
    falls on target_date at target_hour (local time as encoded in the filename).
    """
    ...
    for obj in response['Contents']:
        key = obj.get('Key') or ''
        filename = key.split('/')[-1]
        if filename == f"{experiment_id}_{most_recent['timestamp_str']}_co2-sensor-scd40-0.csv.gz":
            co2_csv_key = key
            break
...
def _list_usbcam_videos_for_experiment_window(
    ...
    """
    List usbcam-0.mp4 objects for the given experiment/box/cage and filter to the experiment window:
    ...
    window_start = datetime.combine(start_date, datetime.min.time()).replace(hour=15, minute=0, second=0)
    ...
    try:
        ...
        raise RuntimeError(f"Error listing S3 objects under s3://{s3_bucket}/{s3_prefix}: {e}")
...
def get_box_video_url(
    ...
    """
    try:
        date_str: Optional[str] = start_date.strip() if isinstance(start_date, str) and start_date.strip() else None
        ...
        # Canonical behavior:
        # - If experiment_id is provided: derive start_date from Airtable and use the experiment window.
        # - Else: use provided start_date (or default PST today) and resolve experiment_id for that start_date.
        ...
        if exp_id:
            ...
            if err or not sd:
                return {'success': False, 'error': err or f'Could not resolve start_date for experiment_id {exp_id}'}
            start_date_effective = sd
        else:
            exp_id = _get_experiment_id_for_box_on_date(api_key, base_id, box_id, start_date_effective)
            if not exp_id:
                return {'success': False, 'error': f'No experiment_id found for box {box_id} on {start_date_effective}'}
        ...
        if not videos:
            return {'success': False, 'error': 'No video available for selected experiment/start_date window'}

        available = [{'timestamp': v['timestamp_str'], 'label': _format_timestamp_label_pst(v['timestamp_str'])} for v in videos]
        ...
        if selected_ts:
            ...
            if not selected_video:
                return {'success': False, 'error': f"Requested timestamp {selected_ts} not found for experiment {exp_id} in start_date window {start_date_effective}."}
        else:
        ...
        # (We can reuse the existing helper by calling _try_find_recent_video_in_hour, but it's cheaper to scan once.)
        co2_csv_key = None
        try:
            # This is consistent with the naming used elsewhere.
            co2_csv_key = f"{exp_id}/{box_id}/{cage_id}/{exp_id}_{selected_video['timestamp_str']}_co2-sensor-scd40-0.csv.gz"
            # Verify existence quickly (head_object is one request). If it fails, we just skip plotting.
...
def _get_experiment_ids_for_box(api_key: str, base_id: str, box_id: str) -> Tuple[Optional[str], Optional[str]]:
    """
...
def _get_experiment_id_for_box_on_date(api_key: str, base_id: str, box_id: str, date_str: str) -> Optional[str]:
    """
    ...
    try:
        _parse_yyyy_mm_dd(date_str)
        table = Api(api_key).table(base_id, 'experiment_planner')
        formula = f"AND({{box_id}} = '{box_id}', IS_SAME({{start_date}}, '{date_str}', 'day'))"
        fields = ['experiment_id', 'start_date']
        records = table.all(formula=formula, fields=fields)
        if not records:
            return None
        # Prefer the first record; if multiple exist, we intentionally don't guess beyond that.
...
def _get_experiment_id_for_box(api_key: str, base_id: str, box_id: str) -> Optional[str]:
    """
    ...
    """
    today_exp_id, yesterday_exp_id = _get_experiment_ids_for_box(api_key, base_id, box_id)
...
def _init_cart_event_cache() -> Path:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to initialize cart event cache dir at {cache_dir}: {e}")
...
def _s3_select_cart_event_rows(
    ...
    for name, v in [('experiment_id', experiment_id), ('box_id', box_id), ('cage_id', cage_id)]:
        if "'" in (v or ''):
            raise ValueError(f"Invalid {name}: contains a single quote which is not supported for S3 Select filters.")
    ...
    for values in reader:
        ...
        if len(values) != len(_CART_EVENT_METADATA_COLUMNS):
            raise ValueError(
                f"Unexpected cart_event_metadata column count {len(values)} (expected {len(_CART_EVENT_METADATA_COLUMNS)}). "
...
def _compute_clip_times_from_row(
    *,
    ...
    if end_frame_id <= start_frame_id:
        raise ValueError(f"Invalid frame range: end_frame_id ({end_frame_id}) <= start_frame_id ({start_frame_id}).")

    duration_s = (end_ts_ms - start_ts_ms) / 1000.0
    if duration_s <= 0:
        raise ValueError(f"Invalid timestamp range: end_timestamp ({end_ts_ms}) <= start_timestamp ({start_ts_ms}).")
...
def get_cart_event_videos(
    *,
    ...
    if not cage_id or not box_id or not date_str:
        return {'success': False, 'error': 'Missing cage_id, box_id, or date parameter', 'status_code': 400}
    ...
    if not api_key or not base_id:
        return {'success': False, 'error': 'Missing AIRTABLE_API_KEY or AIRTABLE_BASE_ID', 'status_code': 500}

    try:
        experiment_id = (experiment_id_override or '').strip() or _get_experiment_id_for_box_on_date(api_key, base_id, box_id, date_str)
        ...
        return {'success': False, 'error': f'Error resolving experiment_id for box {box_id} on {date_str}: {e}', 'status_code': 500}
    ...
    try:
        ...
        return {'success': False, 'error': f'Error initializing S3 client: {e}', 'status_code': 500}
    ...
    try:
        ...
        return {
            'success': False,
            'error': f'Error querying cart event metadata for experiment {experiment_id}, box {box_id}, cage {cage_id}: {e}',
            'status_code': 500,
        }

    def pick_single(rows_in: List[Dict[str, Any]], label: str) -> Tuple[Optional[Dict[str, Any]], Optional[str], int]:
        """
        Select a single row for a clip type.

        Requirement: some (experiment_id, box_id, cage_id, camera_type) combinations have multiple rows.
        We choose the *first* row (in CSV/S3 Select return order) that has a non-empty syringe_qr_frame_id_1.
        If none have syringe_qr_frame_id_1, we fail loud.
        """
        if not rows_in:
            return None, f'No {label} row found in cart_event_metadata for experiment {experiment_id}, box {box_id}, cage {cage_id}.', 404
        ...
        return (
            None,
            f'No {label} row with syringe_qr_frame_id_1 found in cart_event_metadata for experiment {experiment_id}, box {box_id}, cage {cage_id}.',
            404,
        )

    cart_box_rows = [r for r in rows if (r.get('camera_type') or '').strip() == 'cart_box']
    cart_injection_rows = [
        r for r in rows
    ...
    try:
        ...
        cart_box_times = _compute_clip_times_from_row(
            row=cart_box_row,
            chosen_start_frame_id=_to_float_strict('start_frame_id', cart_box_row.get('start_frame_id')),
        )

        # cart_injection clip uses syringe_qr_frame_id_1 -> end_frame_id
        syringe_start = _to_float_strict('syringe_qr_frame_id_1', inj_row.get('syringe_qr_frame_id_1'))
        ...
        return {
            'success': False,
            'error': f'Error computing cart clip times/presigned URLs for experiment {experiment_id}, box {box_id}, cage {cage_id}: {e}',
            'status_code': 500,
        }
...
def generate_cart_event_clip_file(
    *,
    ...
    """
    Generate a trimmed MP4 clip file for either cart_box or cart_injection and return a temp path.

    "Good enough" trimming: time-based using ffmpeg -ss/-t and stream copy (-c copy).
    This is not guaranteed frame-accurate (keyframe limitations), but avoids loading full-hour videos in the browser.
    """
    kind_norm = (kind or '').strip().lower()
    if kind_norm not in ('cart_box', 'cart_injection'):
        return {'success': False, 'error': f"Invalid kind '{kind}'. Expected 'cart_box' or 'cart_injection'.", 'status_code': 400}
    ...
    if not isinstance(clip, dict) or not clip.get('video_url'):
        return {'success': False, 'error': f"Missing {kind_norm} clip data for experiment {meta.get('experiment_id')}, box {box_id}, cage {cage_id}.", 'status_code': 500}

    try:
        start_s = float(clip.get('clip_start_seconds'))
        end_s = float(clip.get('clip_end_seconds'))
    except Exception:
        return {'success': False, 'error': f"Invalid clip timing for {kind_norm} (start/end seconds missing or non-numeric).", 'status_code': 500}

    if not (start_s >= 0 and end_s > start_s):
        return {'success': False, 'error': f"Invalid clip timing for {kind_norm}: start={start_s}, end={end_s}.", 'status_code': 500}

    video_s3_path = str(clip.get('video_s3_path')).strip()
    if not video_s3_path:
        return {'success': False, 'error': f"Missing video_s3_path for {kind_norm}.", 'status_code': 500}
    ...
    # IMPORTANT: Frame IDs in cart_event_metadata.csv are treated as absolute frame indices
    ...
    try:
        ...
        return {'success': False, 'error': f"Missing/invalid start_frame_id or end_frame_id for {kind_norm}.", 'status_code': 500}

    if kind_norm == 'cart_injection':
        try:
            clip_start_frame = int(float(str(clip.get('syringe_qr_frame_id_1')).strip()))
        except Exception:
            return {'success': False, 'error': "Missing/invalid syringe_qr_frame_id_1 for cart_injection.", 'status_code': 500}
    else:
    ...
    if clip_start_frame < 0 or clip_end_frame < clip_start_frame:
        return {
            'success': False,
            'error': f"Invalid frame range for {kind_norm}: start={clip_start_frame}, end={clip_end_frame}.",
            'status_code': 500,
        }

    clip_key = f"{meta.get('experiment_id')}|{box_id}|{cage_id}|{kind_norm}|{video_s3_path}|{clip_start_frame}|{clip_end_frame}"
    ...
    try:
        tmp = tempfile.NamedTemporaryFile(prefix=f"{kind_norm}_", suffix=".mp4", dir=str(cache_dir / 'clips'), delete=False)
        tmp_path = Path(tmp.name)
        tmp.close()
    except Exception as e:
        return {'success': False, 'error': f'Failed to create temp file for clip: {e}', 'status_code': 500}
    ...
    try:
        ...
        if not cap.isOpened():
            try:
                ...
                pass
            return {'success': False, 'error': f"Could not open video with OpenCV: {video_s3_path}", 'status_code': 500}
        ...
        if not fps_out or fps_out <= 0:
            ...
            try:
                ...
                pass
            return {'success': False, 'error': f"Could not determine FPS for {kind_norm} clip.", 'status_code': 500}
        ...
        if not ret or first_frame is None:
            ...
            try:
                ...
                pass
            return {'success': False, 'error': f"Could not read start frame {clip_start_frame} for {kind_norm} clip.", 'status_code': 500}
        ...
        if h <= 0 or w <= 0:
            ...
            try:
                ...
                pass
            return {'success': False, 'error': f"Invalid frame dimensions for {kind_norm} clip: {w}x{h}.", 'status_code': 500}

        fourcc_candidates = [
            cv2.VideoWriter_fourcc(*'avc1'),  # H.264 (if available in this OpenCV build)
            cv2.VideoWriter_fourcc(*'mp4v'),  # MPEG-4 Part 2 (widely available)
        ]

        out = None
        for fourcc in fourcc_candidates:
            writer = cv2.VideoWriter(str(tmp_path), fourcc, float(fps_out), (int(w), int(h)))
            if writer.isOpened():
                out = writer
                break
            try:
        ...
        if out is None or not out.isOpened():
            ...
            return {
                'success': False,
                'error': f"Could not open an MP4 VideoWriter for {kind_norm} clip (tried avc1, mp4v).",
                'status_code': 500,
            }
        ...
        try:
            ...
            while current_frame <= clip_end_frame:
                ...
                if frame.shape[0] != h or frame.shape[1] != w:
                    raise ValueError(f'Frame size changed mid-stream ({frame.shape[1]}x{frame.shape[0]}), refusing to encode.')
                out.write(frame)
                current_frame += 1
        finally:
            try:
        ...
        if not tmp_path.exists() or tmp_path.stat().st_size <= 0:
            try:
                ...
                pass
            return {'success': False, 'error': f'OpenCV produced an empty {kind_norm} clip.', 'status_code': 500}

        # Atomically promote temp to cached clip path (or if another request won the race, prefer the cached file).
        try:
            if clip_path.exists() and clip_path.stat().st_size > 0:
                tmp_path.unlink(missing_ok=True)
            else:
            ...
            try:
                ...
                pass
            return {'success': False, 'error': f'Failed to move generated clip into cache: {e}', 'status_code': 500}

        if not clip_path.exists() or clip_path.stat().st_size <= 0:
            return {'success': False, 'error': f'Clip cache write succeeded but file is missing/empty at {clip_path}', 'status_code': 500}
        ...
        try:
            ...
            pass
        return {'success': False, 'error': f'Error generating {kind_norm} clip: {e}', 'status_code': 500}
