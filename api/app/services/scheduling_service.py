...
def precommit_in_progress_resources_date_range(
    ...
    for exp_record in in_progress_experiments:
        try:
            ...
            if not scheduled_start_date and exp_fields.get('earliest_start_date') and exp_fields.get('num_days'):
                try:
            ...
            if scheduled_start_date and scheduled_end_date:
                for d in DateRange(scheduled_start_date, scheduled_end_date).iter_days():
                    if date_range.contains(d):
                        # Update date-keyed aggregates; preserving prior assumptions
                        daily_cages_booked_preview[d] = daily_cages_booked_preview.get(d, 0) + len(assigned_cages)
                        # mice count requires cage lookup; keep it simple here (could be improved)
                        daily_mice_booked_preview[d] = daily_mice_booked_preview.get(d, 0)
            processed_in_progress.append({
                ...
                'status': 'in_progress',
                'scheduled_start_date': scheduled_start_date.strftime('%Y-%m-%d') if scheduled_start_date else 'In Progress',
                'scheduled_end_date': scheduled_end_date.strftime('%Y-%m-%d') if scheduled_end_date else 'In Progress',
                ...
                'unique_manipulation_ids': exp_fields.get('unique_manipulation_ids', []),
            })
...
def precommit_scheduled_resources_date_range(
    ...
    for exp in scheduled_experiments:
        try:
            ...
            overlap = [d for d in DateRange(start, end).iter_days() if date_range.contains(d)]
            if not overlap:
                continue
            ...
            assigned_cage_record_ids = [cage_custom_to_record.get(cc) for cc in assigned_cages if cage_custom_to_record.get(cc)]

            for d in overlap:
                daily_cages_booked_preview[d] = daily_cages_booked_preview.get(d, 0) + len(assigned_cage_record_ids)
            ...
            processed_scheduled.append({
                ...
                'assigned_syringe_colors': {},
                'unique_manipulation_ids': fields.get('unique_manipulation_ids', [])
            })
        except Exception as e:
...
def prepare_cage_heatmap_data(
    ...
    cage_manip_by_date: Dict[str, Dict[date, List[str]]] = defaultdict(lambda: defaultdict(list))
    ...
    if scheduled_experiments:
        for exp in scheduled_experiments:
            ...
            assigned = exp.get('assigned_cages') or fields.get('assigned_cages') or exp.get('cage') or fields.get('cage') or []
            manips = exp.get('assigned_manipulations') or fields.get('assigned_manipulations') or exp.get('manipulations') or fields.get('manipulations') or []
            start_str = exp.get('scheduled_start_date') or fields.get('scheduled_start_date') or exp.get('earliest_start_date') or fields.get('earliest_start_date')
            end_str = exp.get('scheduled_end_date') or fields.get('scheduled_end_date') or start_str
    ...
    for cage in all_cages_data:
        ...
        for day_date in week_dates:
            ...
            cage_manip_row.append(cage_manip_by_date.get(custom_id, {}).get(day_date, []))
...
def fetch_technician_availability_date_range(
    ...
    try:
        ...
        time_min = tz.localize(datetime.combine(date_range.start, time(0, 0))).isoformat()
        time_max = tz.localize(datetime.combine(date_range.end + timedelta(days=1), time(0, 0))).isoformat()
        ...
        availability_by_date: Dict[date, List[Tuple[str, int]]] = {d: [] for d in date_range.iter_days()}

        name_mapping = {
            'henry': 'Henry', 'angie': 'Angie', 'james': 'James', 'kevin': 'Kevin',
            'tom': 'Tom', 'david': 'David', 'tim': 'Tim', 'gina': 'Gina'
        }
