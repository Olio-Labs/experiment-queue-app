...
def estimate_time_direct_mapping_from_notes(
    ...
    if not isinstance(parsed_map, dict) or not parsed_map:
        return None, "Notes must contain a valid 'map={...}' for cage mapping. Parsed data invalid."

    custom_cage_ids_in_map = set()
    for cage_list_or_item in parsed_map.values():
        if isinstance(cage_list_or_item, list):
            custom_cage_ids_in_map.update(str(c) for c in cage_list_or_item if isinstance(c, (str, int)))
    ...
    if len(manipulation_ids_in_map) == 1 and manipulation_ids_in_map[0] == 'm0000004':
        ...
        all_cages_by_custom_id = {cage.get('fields', {}).get('cage'): cage for cage in all_cages_data}
        ...
        logger.info(
            f"m0000004 experiment: {total_mice_in_experiment} mice * ({health_check_time} + {mouse_weight_time}) = {total_time}"
        )
...
def estimate_time_pseudorandom(
    ...
    if (manip_custom_ids_list and len(manip_custom_ids_list) == 1 and manip_custom_ids_list[0] == 'm0000004'):
        ...
        logger.info(
            f"m0000004 pseudorandom: {num_mice_in_experiment} mice * ({health_check_time} + {mouse_weight_time}) = {total_time}"
        )
        return round(total_time, 2), None

    total_num_cages = 0
    if (manip_custom_ids_list and all_manipulations_map and all_drugs_map and manip_name_to_record_id_map):
        for manip_custom_id in manip_custom_ids_list:
            ...
            if not manip_detail or not isinstance(manip_detail, dict) or 'fields' not in manip_detail:
                total_num_cages += cages_per_manip_general
                continue
            ...
            for drug_record_id in drug_record_ids:
                ...
                if drug_detail and isinstance(drug_detail, dict) and 'fields' in drug_detail:
...
def estimate_time_from_tasks(
    ...
    if assignment_type == 'direct_mapping' and notes:
        ...
        return None, f"Unknown assignment type '{assignment_type}' for task-based calculation."
    ...
    for task in selected_tasks:
        ...
        if task == 'mri':
            ...
            task_time = mri_setup + mri_cleanup + (mri_collect * total_mice_in_experiment)
