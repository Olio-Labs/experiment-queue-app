...
def prepare_tech_workload_chart(
    ...
    booked_hours = [
        (
            daily_tech_time_booked_preview.get(days_of_week_ordered[d.weekday()], 0.0)
            or 0.0
        )
        / 60.0
        for d in week_dates
    ...
    available_hours = [
        (
            daily_tech_details.get(days_of_week_ordered[d.weekday()], {}).get(
...
def prepare_cage_usage_chart(
    ...
    booked_boxes = [
        float(
            len(
                daily_boxes_booked_preview.get(days_of_week_ordered[d.weekday()], set())
                or set()
            )
        )
        for d in week_dates
    ...
    booked_nonbox_cages = [
        float(
            daily_nonbox_cages_booked_preview.get(days_of_week_ordered[d.weekday()], 0)
            or 0
        )
        for d in week_dates
    ]
